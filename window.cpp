    ...
    ui(new Ui::Window)
{
    ...
    connect(ui->rotXSlider, SIGNAL(sliderMoved(int)), this, SLOT(scrollXRotation(int)));
    connect(ui->rotYSlider, SIGNAL(sliderMoved(int)), this, SLOT(scrollYRotation(int)));
    connect(ui->rotZSlider, SIGNAL(sliderMoved(int)), this, SLOT(scrollZRotation(int)));

    connect(ui->pushButtonColor, SIGNAL(clicked()), this, SLOT(ButtonsetColor()));
    ...
    connect(ui->comboBox, SIGNAL(currentIndexChanged(int)), this, SLOT(setTChooseFigure(int)));
}
...
void Window::resizeEvent(QResizeEvent *event)
{
    ...
    ui->myGLWidget->setGeometry(235, 5, w - 239, h - 10);
}

void Window::scrollXRotation(int angle)
{
    ui->myGLWidget->setXRotation(angle);
}

void Window::scrollYRotation(int angle)
{
    ui->myGLWidget->setYRotation(angle);
}

void Window::scrollZRotation(int angle)
{
    ui->myGLWidget->setZRotation(angle);
}

void Window::ButtonsetColor()
{
    QColor oldColor = ui->myGLWidget->colorWall;
    QColor color = QColorDialog::getColor(oldColor);
    if (!color.isValid())
        color = oldColor;
    ui->myGLWidget->setColorWall(color);
}
...
{
    int asix = -1;
    if (ui->radioButtonXrot->isChecked()) asix = 0;
    if (ui->radioButtonYrot->isChecked()) asix = 1;
    if (ui->radioButtonZrot->isChecked()) asix = 2;
    ui->myGLWidget->rotation(sideRotate, asix);
}
...
void Window::setTChooseFigure(int item)
{
    bool a, b, c;
    QString fU, bU, fV, bV;
